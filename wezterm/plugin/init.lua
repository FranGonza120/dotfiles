...
local function apply_to_config(config)
	...
	local status_text = require("ui_mode").get_mode_status_text(left_separator, config.colors.ansi[2], fg_status_color)

	add_mode("UI", require("ui_mode").key_table, status_text)

	status_text = require("scroll_mode").get_mode_status_text(left_separator, config.colors.ansi[7], fg_status_color)
	add_mode("Scroll", require("scroll_mode").key_table, status_text)

	status_text = require("copy_mode").get_mode_status_text(left_separator, config.colors.ansi[4], fg_status_color)

	add_mode("copy_mode", require("copy_mode").key_table, status_text)

	status_text = require("search_mode").get_mode_status_text(left_separator, config.colors.ansi[6], fg_status_color)

	add_mode("search_mode", require("search_mode").key_table, status_text)

	status_text = require("visual_mode").get_mode_status_text(left_separator, config.colors.ansi[3], fg_status_color)
